...
class ArmyCard(commands.Cog):
    ...
    async def CreateCard(self, ctx, name, join_date, exit_date):
        import os
        ...
        elif remain_day.days > 100 or random.randint(1, 100) <= 90:
            filename = random.choice(os.listdir(self.img_path))
            file = discord.File(f"{self.img_path}/{filename}", filename=filename)
    ...
    async def 버터쿠키(self, ctx, *input):
        if ctx.guild.id in [631471244088311840]:
            ...
            await self.CreateCard(ctx, "버터쿠키", join_date, exit_date)
    ...
    async def 뮤(self, ctx, *input):
        if ctx.guild.id in [631471244088311840]:
            ...
            await self.CreateCard(ctx, "뮤", join_date, exit_date)
    ...
    async def 공나물(self, ctx, *input):
        if ctx.guild.id in [631471244088311840]:
            ...
            await self.CreateCard(ctx, "공나물", join_date, exit_date)
