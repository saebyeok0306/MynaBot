...
async def blackJackMessage(message, bot, *input):
    if(message.channel.id == chatChannel):
        try:
            id = message.author.id
            if game_check(id) is False: # 회원가입이 된 유저인지 체크
                embed = discord.Embed(title = f':exclamation: {gameName4} 미가입', description = f'{message.author.mention} {gameName4} 게임에 가입하셔야 이용이 가능합니다. (!회원가입)', color = 0xff0000)
                embed.set_footer(text = f"{message.author.display_name} | {gameName4}", icon_url = message.author.avatar_url)
            ...
            try: betting = int(input[0])
                ...
                embed = discord.Embed(title = f':exclamation: {gameName4} 오류', description = f'{message.author.mention} 배팅하실 금액을 입력하셔야 합니다.\n!블랙잭 [배팅금액]', color = 0xff0000)
                embed.set_footer(text = f"{message.author.display_name} | {gameName4}", icon_url = message.author.avatar_url)
                await message.channel.send(embed = embed)
                return 0
            ...
            if blackJackPlay == 2:
                embed = discord.Embed(title = f':exclamation: {gameName4} 참가불가', description = f'{message.author.mention} 게임이 시작했어요..', color = 0xff0000)
                embed.set_footer(text = f"{message.author.display_name} | {gameName4}", icon_url = message.author.avatar_url)
                await message.channel.send(embed = embed)
                return 0

            if myMoney < betting: # 보유한 돈이 부족할때
                embed = discord.Embed(title = f':exclamation: {gameName4} 금액부족', description = f'{message.author.mention} 보유하고 계시는 돈이 부족합니다.\n보유재산 `{printN(myMoney)}원` :money_with_wings:', color = 0xff0000)
                embed.set_footer(text = f"{message.author.display_name} | {gameName4}", icon_url = message.author.avatar_url)
                await message.channel.send(embed = embed)
                return 0
            ...
            if blackJackPlay == 1:
                if id not in blackJackJoin:
                    ...
                    blackJackUser.append([message.author.display_name, betting, message.author])
                ...
                embed.set_footer(text = f"방장 {message.author.display_name} | {gameName4}", icon_url = message.author.avatar_url)
                embed.add_field(name=f'{message.author.display_name}', value=f'배팅금액 : {printN(betting)}원')
                ...
                while timer:
                    ...
                    for user in blackJackUser:
                        embed.add_field(name=f'{user[0]}', value=f'배팅금액 : {printN(user[1])}원')
                    await msg.edit(embed = embed)
                    try:
                ...
                def printCardField(embed, titleText, logText, show):
                    ...
                    for i in range(joinNum):
                        ...
                        if stopSign[i] > 0: valueText += f'배팅 {printN(userBetting)}원\n결과 {printN(int(userBetting*wdRate[stopSign[i]]))}원\n'
                        else: valueText += f'배팅 {printN(userBetting)}원\n'
